...
# Conclusion
budget_final <- ggplot(data = scaleddata, aes(x = budget, y = cast_popularity, col = vote_average)) +
  viridis::scale_color_viridis(discrete = TRUE) +
  geom_point(size = 0.7, alpha = 0.8, position = "jitter") +
  geom_smooth(method = lm, se = FALSE, size = 2, alpha = 0.8) +
  theme_minimal()
budget_final + facet_wrap(~vote_average)

runtime_final <- ggplot(data = scaleddata, aes(x = runtime, y = cast_popularity, col = vote_average)) +
  viridis::scale_color_viridis(discrete = TRUE) +
  geom_point(size = 0.7, alpha = 0.8, position = "jitter") +
  geom_smooth(method = lm, se = FALSE, size = 2, alpha = 0.8) +
  theme_minimal()
runtime_final + facet_wrap(~vote_average)
